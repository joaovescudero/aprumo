...
// Captured at setup() time; read by teardown() to decide whether to stop the container.
// In reuse mode the container is intentionally long-lived — we must not call stop() on it.
let useReuseGlobal = false;
...
export async function setup(project: { provide: (key: string, value: unknown) => void }) {
  ...
  // Separate try/catch scopes: Docker-unavailability and role-creation failures are
  // distinct failure modes and must be diagnosed independently (CR-01 fix / WR-01 fix).
  try {
  ...
  // Container started — now pre-create cluster-global roles once before any worker fork.
